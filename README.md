...
17. If two or more neurons (in a layer) with same inputs and same output neurons have all their weights  initialised to zeros or same values,  then they will have the same weights even after N number of iterations of the BP.  Random initialization is done to avoid this symmetry between the weights of the neurons.

18. **Weights are initialized to small (random) values to avoid too large or small activations i.e. saturation (leading to slow gradient descent) of neurons with sigmoid or tanh activation functions.**
19. **The biases are usually initialized to zeros.**
20. There are 3 main ways of initializing the weights of an NN.
