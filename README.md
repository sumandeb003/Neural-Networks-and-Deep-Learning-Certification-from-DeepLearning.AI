...
4.  **Logistic regression is for performing binary classification.** 
    - **Logistic regression is done by a single neuron with sigmoid activation function ($\sigma(z)=1/1+e^{-z}=e^x/e^x+1$).**
      - Sigmoid function is a probability distribution curve. 
    - **Forward Propagation in logistic regression: $y=\sigma(z)$ where, $z=w_1x_1+w_2x_2+b$**
