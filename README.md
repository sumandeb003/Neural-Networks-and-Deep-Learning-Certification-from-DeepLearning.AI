...
24. $J = f(y_g, y)$ where, $y=w_1x_1+w_2x_2+w_3x_3+....._w_nx_n$; $dJ/dw_i=(dJ/dy)(dy/dz)(dz/dw_i)$ where, $(dJ/dy)=-(y_g/y)+((1-y_g)/(1-y))$ and $(dy/dz)=y(1-y)$ and $(dz/dw_i)=x_i$. Here, $x_i$, $y_d$ are given and $y$ is known from forward propagation. So, the value of $dJ/dw_i$ can be calculated for each $w_i$. **THE MORE THE VALUE OF $dJ/dw_i$, THE MORE SENSITIVE THE LOSS FUNCTION IS TO THAT WEIGHT**. Example: if $dJ/dw_1$ = 3.2 and $dJ/dw_2$ = 0.1, the loss function would change 32x more for a change $\delta$ in $w_1$ than for the same change $\delta$ in $w_2$.
